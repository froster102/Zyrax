          ...
          {showList &&
            <motion.ul
              ...
              {subCategories?.map((category, i) => {
                return <Link to={`/products?category=${category.name}`} key={i}><li className='w-full pt-2 text-nowrap hover:text-[#147efb] transition ease-in-out'>{_.startCase(category?.name)}</li></Link>
              })}
            </motion.ul>
          }
        </AnimatePresence>
