    ...
    return (
        <>
            ...
            {isOpen && <motion.ul
                ...
                {subCategories.map((category, i) => <Link onClick={() => { closeSideBar() }} to={`products?category=${category.name}`} key={i}><li className='pt-4 text-sm' >{_.startCase(category.name)}</li></Link>)}
            </motion.ul>}
        </>
    )
}
